...
class CrelishJsonDataProvider extends Component
{
    ...
    public function __construct($type, $settings = [], $uuid = null)
    {
        ...
        if (!empty($uuid)) {
            $this->uuid = $uuid;
            if ($theFile = @file_get_contents(\Yii::getAlias($this->pathAlias) . $ds . $type . $ds . $uuid . '.json'))
                $this->allModels[] = $this->processSingle(\yii\helpers\Json::decode($theFile), true);
            else
                $this->allModels[] = [];
        } else {

            $dataModels = \Yii::$app->cache->get('crc_' . $type);

            if ($dataModels === false) {

                // $data is not found in cache, calculate it from scratch
                $dataModels = $this->parseFolderContent($this->type);

                // store $data in cache so that it can be retrieved next time
                \Yii::$app->cache->set('crc_' . $type, $dataModels);
            }
